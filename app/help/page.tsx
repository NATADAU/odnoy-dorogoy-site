...
import { EmailContactDialog } from "./email-contact-dialog";
  ...
  return (
    <>
      ...
      <section className="section help-grid">
        <article className="help-card help-card-coral" id="volunteer"><span>01</span><h2>{clear ? "Стать волонтёром" : "Помочь временем и опытом"}</h2><p>{clear ? "Напишите нам. Расскажите, что вы умеете и когда можете помогать." : "Расскажите, чем вы хотели бы заниматься, какие у вас навыки и сколько времени вы готовы уделять. Мы обсудим подходящий формат участия."}</p><EmailContactDialog label="Написать о волонтёрстве" subject="Хочу стать волонтёром" buttonClassName="button-dark" /></article>
        <article className="help-card help-card-mint" id="partner"><span>02</span><h2>{clear ? "Предложить поддержку" : "Стать партнёром"}</h2><p>{clear ? "Организация может помочь делом, материалами или услугами." : "Если вы представляете компанию или сообщество, можно предложить материалы, профессиональную помощь, площадку или совместный проект."}</p><EmailContactDialog label="Предложить партнёрство" subject="Предложение о партнёрстве" buttonClassName="button-light" /></article>
